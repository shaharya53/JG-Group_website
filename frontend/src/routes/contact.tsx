...
export const Route = createFileRoute("/contact")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Get in touch with Jigisha Group. Headquarters in Bhopal, MP. 24/7 availability." },
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        if (res2.ok) {
          const d = await res2.json(); const pl = d.places[0];
          setForm(p => ({ ...p, city: pl["place name"], state: pl.state, country: d.country }));
        }
      } catch { /* silent */ }
    }, 800);
    ...
    <div style={{ background: "white", minHeight: "100vh" }}>
      ...
      <section style={{ position: "relative", minHeight: "90vh", display: "flex", alignItems: "center", overflow: "hidden", background: BG_DARK }} className="pt-32 pb-24 md:pt-40 md:pb-40 lg:pt-48 lg:pb-48">
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
          <img src={aboutImg} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: 0.3, filter: "grayscale(1)" }} />
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.6 }}
          ...
          {[
            ...
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
              <CountUp value={s.num} className="text-white" style={{ fontSize: "clamp(1.5rem, 4vw, 2rem)", fontWeight: 900, lineHeight: 1 }} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[0.8fr_1.2fr]" style={{ background: "white", borderRadius: "3rem", overflow: "hidden", boxShadow: "0 50px 100px -20px rgba(0,0,0,0.05)", border: "1px solid #e2e8f0" }}>
            ...
            <motion.div initial={{ opacity: 0, x: -32 }} animate={splitIn ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7 }}
              ...
              <div style={{ margin: "4rem 0", display: "flex", flexDirection: "column", gap: "2rem" }}>
                {[
                  { icon: <Phone size={16} />, label: "Call Us Directly",      val: "(+91) 8989167888" },
                ...
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField label="Mobile Number">
                      <input style={fieldInput} type="tel" name="phone" value={form.phone} onChange={handleChange} placeholder="+91 999 888 7777" onFocus={e => { e.target.style.borderColor = "#3b82f6"; e.target.style.background = "white"; }} onBlur={e => { e.target.style.borderColor = "transparent"; e.target.style.background = "#f1f5f9"; }} />
                    </FormField>
                  ...
                  <FormField label="ZIP / PIN Code" hint="— auto-populates location">
                    <input style={fieldInput} type="text" name="pincode" value={form.pincode} onChange={handleChange} placeholder="e.g. 462001" onFocus={e => { e.target.style.borderColor = "#3b82f6"; e.target.style.background = "white"; }} onBlur={e => { e.target.style.borderColor = "transparent"; e.target.style.background = "#f1f5f9"; }} />
                  </FormField>
      ...
      <section ref={mapRef} style={{ padding: "120px 0", background: "white" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 1.5rem" }}>
          <motion.div initial={{ opacity: 0, y: 24 }} animate={mapIn ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }}
            ...
            <div>
              <h2 className="text-4xl md:text-5xl" style={{ fontWeight: 900, letterSpacing: "-0.02em", color: "#0f172a" }}>
                Global <span style={{ color: "#3b82f6" }}>Footprint</span>
              </h2>
              <p style={{ fontSize: "1.125rem", color: "#64748b", marginTop: "0.75rem" }}>
            ...
            <a href="https://www.google.com/maps?q=23.2599,77.4126" target="_blank" rel="noopener noreferrer"
              className="w-fit"
              style={{ padding: "1rem 1.5rem", background: "#f1f5f9", color: "#0f172a", fontWeight: 800, fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.1em", borderRadius: "1rem", display: "flex", alignItems: "center", gap: "0.75rem", textDecoration: "none" }}>
