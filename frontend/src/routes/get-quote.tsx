...
import {
  Clock, Shield, Truck, RefreshCw, Check, Phone, Mail,
  ArrowRight, CheckCircle, Loader2, Paperclip, ChevronDown, MapPin, X,
} from "lucide-react";
...
export const Route = createFileRoute("/get-quote")({
  head: () => ({
    meta: [
      { title: "Get a Quote — Jigisha Group of Companies" },
      { name: "description", content: "Request a competitive quote for industrial procurement, railway services, defence supply, green energy and more. 24-hour response guaranteed." },
      { property: "og:title", content: "Get a Quote — Jigisha Group" },
    ],
  }),
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 mb-5 pb-3 border-b border-border">
      <span className="w-1 h-3.5 rounded-full bg-linear-to-b from-gold/60 to-gold shrink-0" />
      <span className="text-[11px] font-extrabold text-gold tracking-[0.18em] uppercase">{children}</span>
    </div>
  );
}
...
function GetQuotePage() {
  ...
  return (
    <main>
      ...
      {/* ── HERO ───────────────────────────────────────────────── */}
      <section className="relative bg-[oklch(0.07_0.04_250)] overflow-hidden pt-28 pb-36">
        <div
          className="absolute inset-0 opacity-[0.03] pointer-events-none"
          style={{ backgroundImage: "radial-gradient(circle,white 1px,transparent 1px)", backgroundSize: "32px 32px" }}
        />
        <div className="absolute top-0 right-0 w-150 h-150 rounded-full bg-gold/5 blur-[130px] pointer-events-none" />
        <div className="absolute bottom-0 left-1/4 w-100 h-100 rounded-full bg-gold/3 blur-[120px] pointer-events-none" />

        <div className="relative max-w-7xl mx-auto px-4 md:px-8">
          <p className="text-gold text-[11px] font-bold tracking-[0.22em] uppercase mb-4">Request Pricing</p>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 -mt-12">
        <div className="grid grid-cols-2 md:grid-cols-4 bg-card rounded-2xl shadow-xl border border-border overflow-hidden">
          {BENEFITS.map(({ Icon, title, desc }, i) => (
            <div
              key={title}
              className={`flex items-start gap-3.5 p-5 hover:bg-gold/3 transition-colors
                ${i < BENEFITS.length - 1 ? "border-r border-border" : ""}
                ${i < 2 ? "border-b md:border-b-0 border-border" : ""}`}
            >
              <div className="w-10 h-10 rounded-xl bg-navy flex items-center justify-center shrink-0 shadow-sm">
                  ...
                  <form onSubmit={handleSubmit} noValidate>
                      ...
                      <div className="mt-4">
                        ...
                        <label
                          className={`flex items-center gap-3 cursor-pointer px-4 py-3 rounded-xl border-2 border-dashed transition-all duration-200
                            ${attachError ? "border-red-400 bg-red-50" : attachment ? "border-green-400 bg-green-50" : "border-border bg-background hover:border-gold/40"}`}
                        >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-5 border-t border-border">
                      <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                        <label className="flex items-start gap-2.5 cursor-pointer text-sm text-muted-foreground leading-relaxed">
                          ...
                          <span>
                            ...
                            <Link to="/contact" className="text-navy underline hover:text-gold transition-colors">
                              Privacy Policy
                            </Link>.
                          </span>
                        </label>
                      ...
                      <button
                        type="submit"
                        disabled={sending}
                        className="inline-flex items-center gap-2 px-8 py-3.5 bg-gold text-gold-foreground font-bold rounded-full hover:opacity-90 active:scale-[0.98] transition-all text-sm shadow-lg shadow-gold/20 disabled:opacity-60 shrink-0"
                      >
                        {sending ? (
            ...
            <aside className="flex flex-col gap-5 lg:sticky lg:top-24">
                ...
                <ul className="flex flex-col gap-3">
                  {WHY_JIGISHA.map((item) => (
                    <li key={item} className="flex items-start gap-2.5 text-sm text-foreground/70 leading-snug">
                      <Check className="w-3.5 h-3.5 text-navy shrink-0 mt-0.5 stroke-[2.5]" />
                      {item}
                    </li>
                  ))}
                </ul>
                ...
                <div className="flex flex-col gap-3.5">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <a href="tel:+918989167888" className="text-sm font-bold text-foreground hover:text-gold transition-colors">
                        (+91) 8989 167 888
                      </a>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <a href="mailto:info@jigisha.in" className="text-sm font-semibold text-navy hover:text-gold transition-colors">
                        info@jigisha.in
                      </a>
                    </div>
                  </div>
              ...
              <div className="bg-card rounded-2xl border border-border shadow-sm p-6 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
                <h3 className="text-base font-extrabold text-foreground mb-1">
                  Government &amp; <span className="text-navy">Enterprise Procurement</span>
                </h3>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2.5 text-xs text-muted-foreground">
                    <span className="w-5 h-5 rounded-md bg-navy/8 text-navy text-[9px] font-extrabold flex items-center justify-center shrink-0">G</span>
                    GeM-listed vendor — all departments &amp; PSUs
                  </div>
                  <div className="flex items-center gap-2.5 text-xs text-muted-foreground">
                    <span className="w-5 h-5 rounded-md bg-navy/8 text-navy text-[9px] font-extrabold flex items-center justify-center shrink-0">IR</span>
                    IREPS empanelled — all 17 railway zones
                  </div>
                  <div className="flex items-center gap-2.5 text-xs text-muted-foreground">
                    <span className="w-5 h-5 rounded-md bg-navy/8 text-navy text-[9px] font-extrabold flex items-center justify-center shrink-0">B2</span>
              ...
              <Link
                to="/contact"
                className="inline-flex items-center justify-center gap-2 px-6 py-3.5 bg-navy text-white font-bold rounded-full hover:opacity-90 hover:-translate-y-0.5 transition-all text-sm shadow-md"
              >
